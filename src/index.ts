 ...
 * @param options.enableToolCallContext - Injects a "context" parameter to existing tools to capture user intent. The context parameter is appended after the injected `task_id`/`agent_id` parameters.
 ...
 * @param options.resolveTaskId - Hook mode: supply your own task identifier per request (e.g. from your auth or workflow state) and AgentCat steps back — no `task_id` parameter is injected and no task instructions are prompted to the agent. The returned string is combined with your project ID into a deterministic KSUID, so the same identifier always maps to the same task. Return null to mint silently (avoid: the agent can never learn a silently minted ID). Receives the same `(request, extra)` arguments as `identify`.
 ...
 *
 * @param serverOrSessionId - Either a tracked MCP server instance or a task ID string.
 *   A task ID string is used verbatim as the event's session — it is never derived or hashed.
 * @param projectId - Your AgentCat project ID (required)
 * @param eventData - Optional event data to include with the custom event. Set `eventData.taskId`
 *   to attribute the event to a task; it takes precedence over a task ID string passed as the
 *   first argument. When a tracked server is passed without `eventData.taskId`, the event is
 ...
 * @example
 * ```typescript
 * // With a tracked server and no task ID: published without a session
 * await agentcat.publishCustomEvent(
 *   server,
 *   "proj_abc123xyz",
 *   {
...
export async function publishCustomEvent(
  ...
  if (isServer) {
    ...
    if (eventData?.taskId) {
      sessionId = eventData.taskId;
    } else {
      // Handles are per-request now; a tracked server has no ambient task ID.
      sessionId = ""; // wire: null ("stateless mode - server assigns session")
      writeToLog(
        "publishCustomEvent: no taskId provided; event will be published without a session. Pass eventData.taskId to attribute it to a task.",
      );
    }
  } else if (typeof serverOrSessionId === "string") {
    // The string IS the task id — verbatim, no derivation.
    sessionId = eventData?.taskId || serverOrSessionId;
  } else {
    throw new Error(
